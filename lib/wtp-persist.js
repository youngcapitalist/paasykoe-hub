...
export function persistHubOffer({ token, priceEur, vipPriceEur, examCode, liveMasterclasses }) {
  if (typeof window === "undefined" || !token) return;
  try {
    localStorage.setItem(
      HUB_KEY,
      JSON.stringify({ token, priceEur, vipPriceEur, examCode, liveMasterclasses: !!liveMasterclasses, savedAt: Date.now() })
    );
